...
- 关键帧提取：在指定时间点（0/3/6/9/12 秒）截取高质量静态图像，用于后续多模态分析。
- 音频提取：截取视频前 15 秒音频，统一采样率与声道，便于本地 Whisper 转写。
...
- FFmpeg 音频提取：截取前 15 秒音频，统一采样率与声道，输出标准 PCM。
...
U->>R : 提供视频文件路径
R->>O : 创建输出目录
R->>F : 执行关键帧提取0/3/6/9/12s
F-->>O : 生成 frames/*.jpg
R->>F : 执行音频提取前15秒，16kHz，单声道，PCM
F-->>O : 生成 audio.wav
R->>T : 调用 Whisper 转写
...
- 时间点规划：在 0、3、6、9、12 秒五个时间点截取关键帧，形成时间序列样本，便于多模态分析。
- FFmpeg 精确 seek 技术：
...
### 音频提取流程：采样率、声道与格式
- 目标：提取视频前 15 秒音频，统一为 16kHz、单声道、PCM 格式，便于 Whisper 转写。
- FFmpeg 参数要点：
  - -t 15：限定时长为 15 秒。
  - -vn：禁用视频流，仅处理音频。
  - -ac 1：强制单声道，降低复杂度与存储。
...
### 多模态分析与 JSON 解析
- 输入准备：读取 output/frames/ 下的 5 张关键帧（frame_00.jpg ~ frame_04.jpg），并读取 output/transcript.txt。
...
- 音频提取（前 15 秒，16kHz，单声道，PCM）
  - 命令要点：-t 15 限制时长；-vn 禁用视频；-ac 1 单声道；-ar 16000 采样率；-acodec pcm_s16le PCM。
  - 参考
