...
- 音频提取范围从前30秒扩大到前30秒
- 多模态分析提示词从"前30秒"更新为"前30秒"
- HTML模板中的"前30秒"提示更新为"前30秒"
- 关键帧提取时间点从0/3/6/9/12秒扩展到0/1/2/3/.../29秒
- 配置文件中MAX_API_FRAMES从5调整为10
- **新增**：改进了run.sh同步逻辑，增强macOS扩展属性兼容性，采用更精细的文件级复制策略替代递归目录操作
...
- **新增**：跨平台同步：将设计素材同步到落地页管理仓库，采用文件级复制策略增强macOS兼容性。
...
- run.sh：主流程编排，负责调用 FFmpeg 截帧与音频提取、运行 Python 脚本完成转写与分析、最终生成设计方案，并处理跨平台同步逻辑。
...
- assets/categories/{category}/*：系统提示词模板，分别用于多模态分析与设计方案生成。
...
A --> G["跨平台同步<br/>macOS兼容性"]
E --> H["output/analysis.json"]
D --> I["output/transcript.txt"]
B --> J["output/frames/*.jpg"]
C --> K["output/audio.wav"]
F --> L["output/landing_page_design.md"]
F --> M["output/landing_page_design.html"]
G --> N["落地页管理仓库"]
```

**图表来源**
- [run.sh:108-173](file://run.sh#L108-L173)
- [run.sh:175-196](file://run.sh#L175-L196)
...
- [config.env:1-26](file://config.env#L1-L26)
...
- **新增**：跨平台同步：将设计素材逐文件同步到落地页管理仓库，解决macOS扩展属性兼容性问题。
...
## 架构总览
下图展示了从视频输入到最终输出的完整数据流与控制流，包括新增的跨平台同步环节。
...
- **输入准备**：读取 output/analysis.json 与 assets/categories/{category}/generate_prompt.md。
...
### 跨平台同步：macOS扩展属性兼容性改进
- **问题背景**：在macOS系统中，目录包含扩展属性（如资源fork、元数据）时，使用递归复制（cp -R）可能导致权限问题和同步失败。
- **解决方案**：采用更精细的文件级复制策略，逐文件处理而非递归目录操作。
- **实现细节**：
  - 使用 cp -f 逐文件覆盖，避免 macOS 扩展属性导致的 rm/cp -R 失败
  - 通过 for 循环遍历每个页面目录和文件，确保精确控制
  - 保持原有的错误处理机制，使用 2>/dev/null || true 确保同步过程的健壮性
- **部署可靠性增强**：
  - 避免了复杂的目录树递归操作
  - 提高了跨平台兼容性，特别是在macOS环境下
  - 减少了因扩展属性导致的同步中断风险

```mermaid
flowchart TD
Start(["开始：设计素材同步"]) --> Check["检查 design_refer 目录是否存在且非空"]
Check --> |否| Skip["跳过同步并警告"]
Check --> |是| IteratePages["遍历每个 pageN 目录"]
IteratePages --> CreateTarget["为目标目录创建对应结构"]
CreateTarget --> IterateFiles["逐文件处理"]
IterateFiles --> CopyFile["使用 cp -f 逐文件复制"]
CopyFile --> ErrorHandle["错误处理：2>/dev/null || true"]
ErrorHandle --> NextFile["下一个文件"]
NextFile --> NextPage["下一个页面"]
NextPage --> Complete["同步完成"]
Skip --> End(["结束"])
Complete --> End
```
...
- **新增**：跨平台同步性能优化
  - 文件级复制策略相比递归目录操作更高效，减少了不必要的目录遍历开销
  - 逐文件处理提高了错误恢复能力，单个文件失败不影响整体同步进程
...
- **新增**：跨平台同步失败
  - 现象：设计素材无法同步到目标仓库，特别是在macOS系统中。
  - 处理：检查目标路径权限，确认使用文件级复制策略而非递归操作；查看具体文件复制错误。
  - 参考
    - [run.sh:175-196](file://run.sh#L175-L196)
...
- **新增**：增强的跨平台兼容性，特别是macOS系统的扩展属性处理能力，通过文件级复制策略确保部署可靠性。

**更新**：本次重大更新将视频采样时长从15秒扩展到30秒，关键帧提取频率从每3秒一帧调整为每1秒一帧，从15帧扩展到30帧，显著提升了分析精度和内容完整性，为后续的多模态分析和设计方案生成提供了更丰富的素材基础。同时，改进了run.sh同步逻辑，增强了macOS扩展属性兼容性，采用更精细的文件级复制策略替代递归目录操作，显著提升了跨平台部署的可靠性。
...
- **新增**：品类配置
  - CATEGORY_NAME、CATEGORY_FOLDER_NAME：用于同步目标目录的分类管理
  - 参考
    - [config.env:23-26](file://config.env#L23-L26)

### 提示词模板
- **多模态分析提示词**：定义结构化输出字段与 JSON 格式约束，现已更新为分析"前30秒"内容。
  - 参考
    - [assets/categories/{category}/analyze_prompt.md:7-9](file://assets/categories/{category}/analyze_prompt.md#L7-L9)
- **方案生成提示词**：定义设计方案的结构与输出要求。
  - 参考
    - [assets/categories/{category}/generate_prompt.md:1-163](file://assets/categories/{category}/generate_prompt.md#L1-L163)

### 新增：跨平台同步配置
- **同步目标路径**：$HOME/workspace/landing-page-manage/${CATEGORY_FOLDER_NAME:-唱歌}/$VIDEO_NAME
- **同步策略**：文件级复制而非递归目录操作，增强macOS扩展属性兼容性
- **错误处理**：使用 2>/dev/null || true 确保单个文件失败不影响整体同步
- **参考**
  - [run.sh:175-196](file://run.sh#L175-L196)