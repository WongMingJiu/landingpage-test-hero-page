...
- 新增 extract_variants 函数，专门处理变体提取逻辑
- 增强 write_design_refer 函数，支持批量生成设计素材文件夹
...
**重大更新**：增强变体提取系统，支持二级（##）和三级（###）标题格式的变体标记，显著提高了文档结构的灵活性和向后兼容性。新增的 extract_variants 函数能够智能识别不同级别的变体标题格式，确保生成的设计方案具有更强的适应性和扩展性。
...
Design --> HTML[HTML输出]
Design --> Variants[变体提取]
Variants --> DesignRefer[设计素材生成]
DesignRefer --> Output[输出目录]
subgraph "中间产物"
Frames
Audio
Transcript
...
**章节来源**
- [generate.py:550-551](file://generate.py#L550-L551)
- [generate.py:554-599](file://generate.py#L554-L599)
...
**重大更新总结**：增强变体提取系统，支持二级（##）和三级（###）标题格式的变体标记，显著提高了文档结构的灵活性和向后兼容性。新增的 extract_variants 函数能够智能识别不同级别的变体标题格式，确保生成的设计方案具有更强的适应性和扩展性。这一更新不仅提升了系统的技术能力，也为未来的功能扩展奠定了坚实的基础。
