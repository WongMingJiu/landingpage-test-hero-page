...
## 简介
本项目是一个端到端的音频转写与落地页设计方案生成流水线，其中音频转写模块负责将视频前30秒的音频转写为文本。该模块基于 Whisper 模型，采用本地推理的方式进行语音识别，并将结果保存为文本文件供后续多模态分析与设计方案生成使用。模块支持通过环境变量配置 Whisper 模型大小，并对模型加载、推理过程中的异常进行健壮处理。
...
PROMPT_A["assets/categories/{category}/analyze_prompt.md"]
PROMPT_G["assets/categories/{category}/generate_prompt.md"]
end
RUN --> TRANS
RUN --> ANA
RUN --> GEN
...
- 提示词模板（assets/categories/{category}/analyze_prompt.md、assets/categories/{category}/generate_prompt.md）：为分析与生成阶段提供结构化的系统提示。
...
- [assets/categories/{category}/generate_prompt.md:1-69](file://assets/categories/{category}/generate_prompt.md#L1-L69)
...
- 功能概述
  ...
  - 使用 ffmpeg 截取关键帧（0/3/6/9/12 秒）与提取前 30 秒音频（单声道、16kHz、PCM 编码）。
...
ExtractFrames --> ExtractAudio["提取前30秒音频(单声道,16kHz,PCM)"]
