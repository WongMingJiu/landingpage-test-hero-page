...
## 更新摘要
**变更内容**
- 增强品牌logo引用系统，支持品牌栏描述的精确使用
- 新增品牌参考图像处理机制，包括品牌栏样式保持和多参考图像支持
- 更新生成提示词模板，强化品牌标识栏的使用规范
- 改进生图Prompt中的品牌logo参考图像说明
...
## 简介
本指南围绕"广告视频到落地页 Hero 区域"的自动化工作流，系统讲解如何定制与优化分析与生成阶段的提示词模板。通过对 analyze_prompt.md 与 generate_prompt.md 的逐段解读，结合 analyze.py、generate.py、generate_image.py、transcribe.py 的实现细节，帮助读者掌握：
...
**更新** 本版本重点介绍了品牌logo引用系统的增强功能，包括品牌栏描述的精确使用和多参考图像的处理机制。
...
**图表来源**
- [run.sh:100-197](file://run.sh#L100-L197)
...
- 生成提示词模板（generate_prompt.md）
  ...
  - **新增**：强化品牌标识栏使用规范，要求使用品牌logo参考图像保持原样设计
- 分析脚本（analyze.py）
...
- 生成脚本（generate.py）
  ...
  - **新增**：生成品牌参考图像（brand_reference.png）供生图阶段使用
- 生成图像脚本（generate_image.py）
...
## 架构总览
该系统以"提示词模板 + 脚本编排"为核心，形成"输入视频 → 多模态分析 → 结构化结果 → 设计方案生成 → 品牌logo参考 → 最终生图输出"的完整闭环。提示词模板是"知识与约束"的载体，脚本负责"数据准备、调用 API、解析与持久化"。
...
GE->>P2 : 读取生成提示词并替换占位符
GE->>GE : 文本消息构造与调用
GE-->>SH : 生成 landing_page_design.md / .html
SH->>GI : 调用生图脚本
GI->>BR : 读取品牌logo参考图像
GI->>GI : 处理多参考图像品牌logo + 老师参考图
GI-->>U : 生成 Hero 页面变体
```
...
**新增品牌logo引用系统增强**
- **品牌标识栏规范**：必须使用提供的品牌logo参考图（brand_logo.png），原样保持其设计风格、颜色和文字排版
- **多参考图像支持**：生图时需同时附带品牌logo参考图和老师形象参考图
- **品牌栏描述**：在生图Prompt中明确要求品牌区域使用提供的品牌logo参考图，保持红色背景、文字排版和配色
...
G->>P : 读取生成提示词并替换占位符
G->>G : 调用文本 API 生成内容
G->>G : 生成品牌参考图像 brand_reference.png
G->>M : Markdown 渲染
M-->>G : HTML 文本
G->>H : 注入内容并输出 HTML
```
...
**图表来源**
- [generate.py:325-372](file://generate.py#L325-L372)
- [generate.py:430-608](file://generate.py#L430-L608)
...
- 数据准备
  - 读取每个变体目录下的 prompt.md
  - 读取对应的参考图像（teacher_ref.jpg 和 brand_reference.png）
- 多参考图像处理
  - 构建 image_paths 列表，包含品牌logo和老师参考图
  - 动态检测品牌logo是否存在，若不存在则仅使用老师参考图
- API调用
  - 调用 /images/edits API，支持多张参考图像
  - 设置适当的超时时间和重试机制
- 输出生成
  - 保存生成的 Hero 页面变体图片
  - 提供详细的日志信息和错误处理
...
```mermaid
flowchart TD
Start(["开始生图"]) --> LoadPrompt["读取变体提示"]
LoadPrompt --> LoadRefs["加载参考图像"]
LoadRefs --> CheckBrandLogo{"检查品牌logo"}
CheckBrandLogo --> |存在| BuildImagePaths["构建参考图列表<br/>品牌logo + 老师参考图"]
CheckBrandLogo --> |不存在| BuildTeacherOnly["仅使用老师参考图"]
BuildImagePaths --> CallAPI["调用 /images/edits API"]
BuildTeacherOnly --> CallAPI
CallAPI --> SaveImage["保存生成的图片"]
SaveImage --> End(["结束"])
```
...
- 步骤编排：截帧 → 提取音频 → 转写 → 分析 → 生成 → 生图
- 产物输出：关键帧、音频、转写文本、分析结果、设计方案（Markdown 与 HTML）、Hero页面变体
...
- 环境变量
  ...
  - IMAGE_API_BASE_URL/IMAGE_API_KEY/IMAGE_MODEL_NAME：**新增**生图阶段配置
...
ENV["环境变量(API_BASE_URL/API_KEY/MODEL_NAME/IMAGE_API_BASE_URL/IMAGE_API_KEY/IMAGE_MODEL_NAME/WHISPER_MODEL)"]
CFG --> ENV
REQ --> ENV
```
...
- **新增**：多参考图像处理优化
  - 动态检测品牌logo存在性，避免不必要的API调用
  - 支持部分参考图像缺失的情况，提供降级处理
...
- [generate_image.py:180-210](file://generate_image.py#L180-L210)

## 故障排除指南
- 缺少环境变量
  - 分析与生成阶段均会检查 API 基础地址、密钥与模型名，缺失时报错并退出
  - **新增**：生图阶段检查 IMAGE_API_BASE_URL、IMAGE_API_KEY、IMAGE_MODEL_NAME
- 关键帧或转写文件缺失
  - 分析阶段会检查关键帧目录与转写文件是否存在
  - 生成阶段会检查分析结果文件是否存在
- API 调用失败
  - 分析与生成阶段均提供重试与指数回退；失败时打印错误并等待一段时间后重试
  - **新增**：生图阶段提供重试机制，支持多参考图像的容错处理
- JSON 解析失败
...
## 结论
本工作流通过"模板化提示词 + 脚本编排"的方式，实现了从广告视频到落地页 Hero 区域的自动化生产。**最新版本增强了品牌logo引用系统，确保品牌视觉的一致性和精确性**。提示词模板是知识与约束的载体，脚本负责数据准备、调用 API 与结果持久化。通过合理设计分析与生成阶段的提示词，可以有效提升结构化信息抽取的准确性与设计方案的可执行性。**新增的品牌logo引用系统为品牌营销提供了更强的视觉保障**。建议在实际应用中持续进行 A/B 测试与效果评估，逐步优化提示词与参数，以满足不同行业与风格的需求。
...
### 如何根据需求调整分析深度与生成质量
- 分析深度
  - 增加维度：如加入"品牌调性""目标人群画像""竞品对比"等
  - 细化字段：对痛点与卖点增加"具体场景/方法论"等子字段
- 生成质量
  - 提升温度：在生成阶段适度提升温度以增强创造性
  - 扩展输出：增加"竞品对比""FAQ""风险提示""品牌故事"等模块
  - **新增**：品牌一致性增强：在生成阶段强化品牌logo引用和品牌元素使用
- 控制成本
  - 降低温度与减少维度，缩短生成时间与成本
  - **新增**：优化品牌logo引用：合理使用品牌参考图像，避免过度复杂的品牌元素

### Prompt 测试与迭代方法
- A/B 测试
  - 对比不同角色设定、任务描述与输出格式，记录生成内容的质量与一致性
  - **新增**：测试品牌logo引用系统的效果，比较使用与不使用品牌参考图像的差异
- 效果评估
  - 人工抽样评估：从生成内容中抽取样本，评估是否符合设计意图与可执行性
  - 自动化指标：统计 JSON 结构完整性、字段覆盖率、文案可读性评分
  - **新增**：品牌一致性评估：检查生成的Hero页面是否正确使用品牌logo
- 迭代路径
  - 基于评估结果优化提示词结构与约束，逐步收敛到稳定版本
  - **新增**：持续优化品牌logo引用系统的使用效果和视觉表现
...
- **新增**：品牌一致性问题
  - 现象：不同变体间的品牌视觉不一致
  - 解决：确保所有变体都使用相同的品牌logo参考图像；检查生成脚本中的品牌参考图像复制逻辑；验证生图时的品牌logo引用