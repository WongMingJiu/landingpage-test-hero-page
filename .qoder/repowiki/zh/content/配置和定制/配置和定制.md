...
这是一个基于多模态AI的视频内容分析和落地页设计自动化工具。该系统能够自动分析视频广告的前30秒内容，提取关键信息并生成专业的落地页Hero区域设计方案。系统支持多种配置选项、可定制的Prompt模板，以及灵活的批量处理能力。
...
subgraph "提示模板"
AnalyzePrompt[assets/categories/{category}/analyze_prompt.md<br/>通用分析模板]
GeneratePrompt[assets/categories/{category}/generate_prompt.md<br/>通用生成模板]
CategoryPrompt[assets/categories/<br/>品类特定模板]
end
subgraph "品类配置系统"
...
BrandRef[assets/categories/{category}/brand_reference.png<br/>品牌参考图]
TeacherFaceRef[assets/categories/{category}/teacher_face_ref_*.jpg<br/>老师脸部三视图]
end
subgraph "输出层"
...
DesignRefer[design_refer/<br/>变体素材文件夹<br/>page1-pageN]
...
### 分析组件 (analyze.py)
负责多模态分析，结合关键帧图片和音频转写文本，调用AI模型提取结构化信息。**更新** 新增帧数采样功能，**新增** 支持营养健康类别的食谱参考帧提取。
...
分析组件实现了robust的错误处理和重试机制，**更新** 新增帧数采样功能，**新增** 支持营养健康类别的食谱参考帧提取：
...
生成组件提供了灵活的HTML渲染机制，**更新** 新增视觉预处理系统集成，**新增** 支持--batch参数的批量处理模式，**新增** 集成多类别配置系统：
...
系统支持从 `assets/categories/{category}/config.json` 加载品类配置：
...
**新增** 系统支持从assets/categories/目录加载品类配置文件：

```mermaid
flowchart TD
Start([开始加载品类配置]) --> CheckExternal{检查品类config.json}
CheckExternal --> |存在| LoadExternal[加载外部config.json]
CheckExternal --> |不存在| ErrorConfig[报错并提示创建配置]
LoadExternal --> ValidateExternal{验证配置格式}
ValidateExternal --> |有效| ReturnExternal[返回外部配置]
ValidateExternal --> |无效| ErrorConfig
...
ApplyBatchMode --> |batch=1| SetPageRange1[设置page1-pageN]
ApplyBatchMode --> |batch=2| SetPageRange2[设置page{N+1}-page{2N}]
...
|-------------|--------|------|----------|----------|
| 品牌参考图 | brand_reference.png / brand_logo.png | 品牌标识栏参考 | 从品类目录复制 | 是 |
| 老师脸部三视图1 | teacher_face_ref_1.jpg | 面部特征一致性参考 | 从品类目录复制 | 是 |
| 老师脸部三视图2 | teacher_face_ref_2.jpg | 面部特征一致性参考 | 从品类目录复制 | 是 |
...
| 唱歌品牌参考 | singing/brand_logo.png | 唱歌品牌参考 | 从品类目录复制 | 是 |
...
CheckAssets --> |不存在| WarnMissing[提示缺少品类素材]
...
CheckCategory --> |其他| LoadCategoryAssets[加载指定品类资产]
LoadNutritionAssets --> CopyToDesign[复制到设计目录]
LoadSingingAssets --> CopyToDesign
LoadCategoryAssets --> CopyToDesign
WarnMissing --> End([完成])
...
品类资源系统通过 `config.json`、Prompt 模板和参考图片提供内容约束，确保不同业务品类可以自包含地配置文案方向、变体数量和视觉素材：

```mermaid
graph TB
subgraph "品类配置"
CategoryConfig[assets/categories/{category}/config.json<br/>品类配置文件]
PainPoints[pain_points<br/>品类痛点列表]
SellingPoints[selling_points<br/>品类卖点列表]
TitlePool[title_pool<br/>老师头衔池]
VariantCount[variant_count<br/>变体数量]
end
subgraph "模板与素材"
AnalyzePrompt[analyze_prompt.md<br/>分析提示词]
GeneratePrompt[generate_prompt.md<br/>生成提示词]
BrandLogo[brand_logo.png<br/>品牌素材]
TeacherRefs[teacher_face_ref_*.jpg<br/>老师脸部参考]
Examples[examples/<br/>风格示例图]
end
subgraph "生成组件集成"
CategoryLoader[category_config.py<br/>加载品类配置]
GeneratePromptFill[generate.py<br/>替换Prompt占位符]
WriteRefer[design_refer/pageN<br/>写入变体素材]
end
CategoryConfig --> CategoryLoader
PainPoints --> GeneratePromptFill
SellingPoints --> GeneratePromptFill
TitlePool --> GeneratePromptFill
VariantCount --> GeneratePromptFill
AnalyzePrompt --> CategoryLoader
GeneratePrompt --> GeneratePromptFill
BrandLogo --> WriteRefer
TeacherRefs --> WriteRefer
Examples --> GeneratePromptFill
```

**图表来源**
- [category_config.py:1-120](file://category_config.py#L1-L120)
- [generate.py:895-965](file://generate.py#L895-L965)

### 品类内容字段

品类配置包含以下核心内容字段：

- `variant_count`：每轮生成的方案数量，用于计算 `pageN` 范围
- `title_pool`：可用老师头衔池
- `content_list_name` / `content_list_description`：课程内容列表命名
- `decoration_elements`：品类相关装饰元素
- `pain_points`：品类痛点候选
- `selling_points`：品类卖点候选

### 品类资源集成流程

```mermaid
flowchart TD
Start([开始品类资源处理]) --> LoadConfig[加载config.json]
LoadConfig --> LoadPrompts[加载analyze/generate Prompt]
LoadPrompts --> ReplacePlaceholders[替换生成Prompt占位符]
ReplacePlaceholders --> CopyAssets[复制品牌和老师参考图]
CopyAssets --> WriteDesignRefer[写入design_refer/pageN]
WriteDesignRefer --> End([完成])
```
...
**图表来源**
- [assets/categories/{category}/analyze_prompt.md:15-153](file://assets/categories/{category}/analyze_prompt.md#L15-L153)

### 生成Prompt模板增强

生成模板包含了新的视觉预处理约束、品类资源配置和多类别配置：
...
CategoryContent[品类内容配置<br/>痛点/卖点候选]
...
**图表来源**
- [assets/categories/{category}/generate_prompt.md:53-56](file://assets/categories/{category}/generate_prompt.md#L53-L56)
- [assets/categories/{category}/generate_prompt.md:131-136](file://assets/categories/{category}/generate_prompt.md#L131-L136)
...
#### batch=1 模式（默认）
- **功能**：首次生成page1-pageN的设计方案
- **特点**：不进行去重检查，直接生成多套不同的设计方案
- **输出**：生成design_refer/page1到pageN目录
- **适用场景**：首次生成落地页设计方案，探索多种创意方向

#### batch=2 模式
- **功能**：基于已有page1-pageN方案进行去重生成下一批page
- **特点**：读取现有多套方案，确保新方案在风格、配色、布局三个维度完全不同
...
1. **更新分析模板**: 在assets/categories/{category}/analyze_prompt.md中添加新的分析维度
...
3. **Prompt模板**: 在assets/categories/目录下添加新品类的Prompt模板
4. **参考图片**: 在assets/categories/目录下添加新品类的参考图片和示例
5. **品类内容**: 添加新品类的品类配置与素材文件
...
4. **模板更新**: 更新assets/categories/{category}/generate_prompt.md以包含新的参考要求
...
#### 创意探索阶段
- **使用batch=1**：生成多套完全不同的设计方案
- **目的**：探索各种创意方向和视觉风格
- **输出**：design_refer/page1-pageN
- **特点**：无去重约束，鼓励创新

#### 方案对比阶段
- **使用batch=2**：基于已有方案生成去重的新方案
- **目的**：确保新方案在风格、配色、布局上完全不同
- **输出**：design_refer/page{N+1}-page{2N}
- **特点**：严格去重约束，保证差异化
...
| 品类内容不足 | 分析结果缺少候选内容 | config.json字段缺失 | 补充pain_points/selling_points等品类字段 |
...
**更新** 新增的多类别配置系统、视觉预处理系统、参考图片系统、品类资源系统、增强的教师图像约束配置和**新增的批量处理功能**大大提升了系统的专业性和可靠性。系统的主要优势包括：
...
- **智能的品类资源系统**: 提供默认的痛点、卖点和利益点模板，支持品类特定内容
...
- **品类内容定制**: 每个品类支持独立的品类内容配置
- **页面偏移计算**: 自动计算不同品类的页面映射关系

建议用户根据具体业务需求调整配置参数，并定期更新Prompt模板以适应不断变化的市场需求。使用.gitignore确保项目结构的整洁，利用config.env.example快速启动新项目。多类别配置系统和品类资源系统为落地页制作提供了更加完整的解决方案，用户可以根据需要调整图像生成的参数和样式要求。教师图像约束配置确保了生成图片的专业性和视觉一致性，特别适用于需要严格视觉规范的商业应用场景。批量处理功能为大规模内容生产提供了高效的技术支撑，用户可以通过合理的batch模式组合实现创意探索和方案优化的双重目标。动态变体计算功能使得不同品类可以按照自身特点优化方案数量，提高内容生产的效率和质量。
