...
**更新** 新版本增强了安装步骤、改进了配置方法，并增加了自动同步功能到落地页管理仓库。
...
export MAX_API_FRAMES=5
```

**更新** 新增了`MAX_API_FRAMES`配置项，用于控制发送给API的最大帧数，以及更详细的配置文件操作步骤。
...
| 参数名称 | 描述 | 默认值 |
|---------|------|--------|
| `API_BASE_URL` | API基础URL | `https://your-api-endpoint.com` |
| `API_KEY` | API访问密钥 | `your-api-key-here` |
| `MODEL_NAME` | 模型名称 | `gpt-5.5` |
| `WHISPER_MODEL` | Whisper模型大小 | `small` |
| `MAX_API_FRAMES` | 发送给API的最大帧数 | `5` |
...
**更新** 新增了`MAX_API_FRAMES`参数说明，用于优化API调用性能和成本控制。

**章节来源**
- [config.env.example:2-5](file://config.env.example#L2-L5)
...
# 控制发送给API的帧数
export MAX_API_FRAMES=3
./run.sh ./sample.mp4
```

**更新** 新增了`MAX_API_FRAMES`的使用示例，展示了如何优化API调用性能。
...
```mermaid
flowchart TD
A["输入视频文件"] --> B["截取关键帧<br/>frame_00.jpg ~ frame_14.jpg"]
B --> C["提取音频<br/>audio.wav (前30秒)"]
C --> D["Whisper本地转写<br/>transcript.txt"]
D --> E["多模态分析<br/>analysis.json"]
E --> F["生成设计方案<br/>landing_page_design.md/.html"]
F --> G["生成设计素材<br/>design_refer/pageN/"]
G --> H["同步到管理仓库<br/>landing-page-manage/"]
I["配置文件<br/>config.env"] --> A
J["环境变量<br/>API配置"] --> E
K["Python依赖<br/>requirements.txt"] --> D
style A fill:#e1f5fe
style B fill:#f3e5f5
style C fill:#f3e5f5
style D fill:#e8f5e8
...
**更新** 新增了设计素材生成和自动同步到管理仓库的两个新阶段。

**图表来源**
- [run.sh:108-197](file://run.sh#L108-L197)
...
# 控制发送给API的帧数
export MAX_API_FRAMES=3  # 减少API调用成本
```
...
echo "管理仓库: $(test -d ~/workspace/landing-page-manage && echo '可写' || echo '权限不足')"
```

**更新** 新增了管理仓库权限检查和同步功能的诊断建议。
...
**更新** 新版本提供了更完善的配置管理和自动同步功能，大大简化了工作流程。
