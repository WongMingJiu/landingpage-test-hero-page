...
以下是从 10 张真实兴趣岛{CATEGORY_DISPLAY_NAME}课程落地页中总结的常见布局**灵感**，你可以参考、混搭或创新：
...
### 灵感 B：歌曲列表型（唱歌品类适用）
- 布局特征：顶部品牌 logo → 大标题 + "中老年轻松入门" → 效果标签 → 老师（右侧）→ 歌曲编号列表（5-6首） → 场景标签（KTV/居家/公园）→ 底部承诺条
...
### 灵感 H：播放器拟物型（唱歌品类适用）
- 布局特征：模拟音乐播放器 UI（圆形播放按钮、进度条、波形图等元素）+ 场景标签（KTV/居家/公园等情境胶囊）+ 老师形象嵌入播放器卡片之中
- 配色建议：暖橙渐变 + 黑金播放器质感（注意：播放器组件可用深色但**不得作为整页主背景**）或 薄荷绿 + 米白机身
- 适用场景：强调"随时随地唱"的生活感、目标用户对音乐场景有共鸣

### 灵感 I：赠品促转型
- 布局特征：卡片式歌曲列表（每首歌一张迷你卡）+ 底部赠品栏（横向滑动卡或徽章式陈列"赠：发声练习手册""赠：呼吸训练音频"）+ 超大价格标（圆形/盾形锚点）
- 配色建议：活力橙红 + 明黄高亮 或 暖金 + 红色赠品标
- 适用场景：促转导向强、价格敏感、需要赠品提升感知价值

### 灵感 J：健康图标列表型
- 布局特征：5 个带图标的健康益处横条（每条左侧大图标，右侧短文案，如"🫁 增强肺活量""💪 改善气血""😊 释放压力""🎤 锻炼咬字""👥 拓展社交"）+ 顶部老师小幅照
...
3. **老师姓名 + 头衔**：
   ...
   - 如果 `teacher.title` 为 null，则每套变体从头衔池中随机选用一个（{TITLE_POOL_JSON}），不同变体可以不同
4. **主标题**（课程核心卖点）
