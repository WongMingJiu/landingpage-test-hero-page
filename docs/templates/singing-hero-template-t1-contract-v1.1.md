...
变更原因：产品 / 实验决策——Hero 不再展示或锁定任何价格信息，Bottom Banner 作为唯一的底部销售收束模块。

1. 删除右下角圆形「1元」价格徽章（Business Locked / Visual Locked / Rendering Contract 同步删除）；
2. 删除底部价格免责声明文案；
3. Bottom Banner 改为完整宽度的底部销售收束模块，视觉重心居中平衡；
4. **12 个 Dynamic Slots 完全不变**（含 `bottom_banner_text` 保持 Dynamic，这不是价格动态化，也不是新增 Slot）；
5. 新增 T1 V1.1 Reference（`v2/templates/hero_template_t1/reference_v1.1.png`）作为版式与视觉真值；Runtime Prompt 不再承担"删除价格圆标"的职责；
6. 禁止 Runtime 自行生成任何价格、优惠或 Offer Badge；
7. 文档一致性：§6.1 Headline 示例 `5天学会` 同步为 `5天入门`（与 fixtures slots_a 现行文案对齐；示例仅说明结构，不构成文案库）。
...
**V1.1 新增：Hero 画面不展示任何价格信息（无价格徽章、无价格说明、无优惠表达）。**
...
左：4 个纵向利益卡      右：宋伶俐老师大幅人物
        ↓
底部完整宽度大型价值 Banner（底部销售收束模块，视觉居中平衡）
```
...
**V1.1：无价格徽章、无价格说明。画面不得出现任何价格、优惠或 Offer 信息。**
...
Bottom Banner 是**完整宽度的底部销售收束模块**，承担 Hero 的最终收束，避免再次堆叠新问题。
...
- 不买后悔 / 错过再无等恐惧诱导；
- **V1.1 新增：任何价格、优惠、折扣或 Offer 表达（Hero 画面不承载价格信息）**。
...
```text
layout_ref = T1 V1.1 Reference Image（v2/templates/hero_template_t1/reference_v1.1.png）
...
如果必须发生以上变化，应建立 `T1 V2`，不得静默修改已冻结版本。
